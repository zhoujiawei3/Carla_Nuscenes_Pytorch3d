...
class Client:
    ...
    def generate_custom_scene(self,scene_config):
        ...
        #设计了对自车的驾驶策略
        seed_value = 0
        self.trafficmanager.set_random_device_seed(seed_value)
        ...
        while reGenerate:
            ...
            if scene_config["vehicles"] is not None:
                for vehicle_config in scene_config["vehicles"]:
                    ...
                    angle = random.uniform(-180, 180) #以车辆正方向为坐标系的0度
                    #角度转换至世界坐标系
                    angle = (angle+ego_rotation["yaw"])*math.pi/180
                    ...
                    for attr in ["yaw"]:
                        vehicle_config["rotation"][attr]=ego_rotation[attr]+random.uniform(-180, 180)
