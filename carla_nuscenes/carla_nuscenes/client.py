...
class Client:
    ...
    def generate_world(self,world_config):
        ...
        self.trafficmanager.set_synchronous_mode(True) #启动交通管理器
        ...
        while reGenerate:
            ...
            if scene_config["vehicles"] is not None:
                for vehicle_config in scene_config["vehicles"]:
                    ...
                    vehicle_config["location"]["x"]=ego_location["x"]+distance*math.cos(angle)
                    vehicle_config["location"]["y"]=ego_location["y"]+distance*math.sin(angle)
            ...
            for i,response in enumerate(self.client.apply_batch_sync(vehicles_batch)):
                if not response.error:
                    ...
                    self.trafficmanager.keep_right_rule_percentage(vehicle_actor, 100)
                    self.trafficmanager.ignore_lights_percentage(vehicle_actor, 100)
                    self.trafficmanager.ignore_signs_percentage(vehicle_actor, 100)
                    self.trafficmanager.distance_to_leading_vehicle(vehicle_actor, 5)  # Important for collision avoidance
                    self.trafficmanager.vehicle_percentage_speed_difference(vehicle_actor, -20)
                    self.trafficmanager.auto_lane_change(vehicle_actor, True)
                    # If you want the vehicles to avoid each other and the ego vehicle
                    self.trafficmanager.ignore_vehicles_percentage(vehicle_actor, 0)
                else:
