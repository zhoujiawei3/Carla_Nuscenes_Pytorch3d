...
class Dataset:
    ...
    def update_sample_data(self,prev,calibrated_sensor_token,sample_token,ego_pose_token,is_key_frame,sample_data,height,width,renderer_vehicle_location,renderer_vehicle_rotation,replace=True):
        ...
        elif sensor['modality'] == "segmentation":
            sample_data_item["fileformat"] = "jpg"
        ...
        # sample_data_item["renderer_vehicle_translation"] = renderer_vehicle_location
        # sample_data_item["renderer_vehicle_rotation"] = renderer_vehicle_rotation
        filename = self.get_filename(sample_data_item)
        save_sensor_data(sample_data[1],os.path.join(self.root,filename))
        if sensor["modality"] == "segmentation":
            #重新读取该文件，并且转换为numpy数组
            ...
            # 创建一个布尔数组来标记需要改变的像素
            mask = (pixels[:, :, 0] == 0) & (pixels[:, :, 1] == 0) & (pixels[:, :, 2] == 142)
            ...
            result_image.save(os.path.join(self.root,filename))
            del pixels
            del mask
            del result_image
            del image
